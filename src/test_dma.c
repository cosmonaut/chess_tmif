...
static void ISR(dm7820_interrupt_info interrupt_status)
{
    ...
    case DM7820_INTERRUPT_FIFO_0_DMA_DONE:
        //printf("FIFO 0 DMA DONE!!\n");
        break;
    case DM7820_INTERRUPT_FIFO_1_DMA_DONE:
        //printf("THIS SOULD NOT SHOW UP!\n");
        break;
    default:
        break;
    }
...
int main(void) {
    ...
    /* Fill dma buffer with tasty nougat center */
    for (i = 0; i < DMA_BUF_NUM; i++) {
        for (j = 0; j < (DMA_BUF_SIZE/2 - 1); j++) {
            switch(j%3) {
            case 0:
                dma_buf[i*256 + j] = (magic_word | 0x2000);
                break;
            case 1:
                dma_buf[i*256 + j] = (magic_word | 0x4000);
                break;
            case 2:
                dma_buf[i*256 + j] = (magic_word | 0x6000);
                break;
            default:
                printf("WTFBBQ?!\n");
                break;
        ...
        magic_word += 10;
        dma_buf[i*256 + 255] = 0x0000;
    }


    // /* transfer 1 dma buffer... */
